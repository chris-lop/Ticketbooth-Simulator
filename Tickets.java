...
public class Tickets {
	...
	// Constructor with 5 integer parameters to set number of each ticket in ticket
	// booth
	...
	// Method that allows one to increase the number of each tickets by the
	// indicated number
	...
	// Method which returns a double indicating the total value of the tickets in
	// the ticket booth
	public Double ticketsTotal() {
		return (this.regularNb * this.REGULAR) + (this.juniorNb * this.JUNIOR) + (this.seniorNb * this.SENIOR)
				+ (this.dailyNb * this.DAILY) + (this.weeklyNb * this.WEEKLY);
	}

	// Method which will return a string clearly indicating the count of each ticket
	// in the ticket booth
	public String toString() {
		return this.regularNb + " x $" + this.REGULAR + " + " + this.juniorNb + " x $" + this.JUNIOR + " + "
				+ this.seniorNb + " x $" + this.SENIOR + " + " + this.dailyNb + " x $" + this.DAILY + " + "
				+ this.weeklyNb + " x $" + this.WEEKLY;
	}

	// Method which will return true if the two objects of type Tickets being
	// compared have the same breakdown of tickets and false otherwise
	public Boolean equals(Tickets tickets) {
		if (this.regularNb == tickets.getRegular() && this.juniorNb == tickets.getJunior()
				&& this.seniorNb == tickets.getSenior() && this.dailyNb == tickets.getDaily()
				&& this.weeklyNb == tickets.getWeekly()) {
			return true;
		} else
			return false;
	}
