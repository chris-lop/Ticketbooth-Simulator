...
public class Ticketbooth {
	...
	// Method that returns true if total value of the tickets of two ticket booth objects are equal, and false otherwise
	public Boolean equalValue(Ticketbooth ticketbooth) {
		return this.totalValue() == ticketbooth.totalValue();
	}
	
	// Method that will return true if number of each type of ticket of two ticket booth objects are equal, and false otherwise
	...
	// Method which will add new OPUS card to a ticket booth, and returns number of OPUS cards after addition
	...
	// Method which will remove OPUS card from ticket booth (needs to account for ticket booth that has no OPUS cards)
	...
	// Method that adds tickets to ticket booth. Returns new total value of tickets' amount in ticket booth.
	...
	// Equals method which returns true if total value of tickets and number of OPUS card tickets of two ticket booths are equal, false otherwise.
	public Boolean equals(Ticketbooth ticketbooth) {
		if (Double.compare(this.totalValue(), ticketbooth.totalValue())==0 && this.opusCount() == ticketbooth.opusCount()) {
			return true;
		} else return false;
	}
	...
	// Method that returns a string clearly indicating number of each ticket as well as details of each OPUS card in ticket booth. Should work for ticket booths that
	// have no OPUS cards
