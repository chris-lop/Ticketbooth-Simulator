...
public class OPUSCard {
	...
	// Constructor with 4 parameters to set the initial value of each attribute (if month is not between 1 and 12, set it to 0)
	...
	// Setters (mutator) methods for expiration month and expiration year. If expiration month is not between 1 and 12, sets it to 0.
	public void setExpiryMonth(int expiryMonth) {
		if (expiryMonth>=1 && expiryMonth<=12) {
			this.expiryMonth = expiryMonth;
		} else this.expiryMonth = 0;
	}
	...
	// Method which will return a string indicating the type of OPUS card, the name of the owner, as well as the expiration date formated as mm/yyyy.
	...
	// Equals method which will return true if two objects of type OPUSCard are identical
	public Boolean equals(OPUSCard newCard) {
		if (this.cardType == newCard.getCardType() && this.name == newCard.getName() && this.expiryMonth == newCard.getExpiryMonth() && this.expiryYear == newCard.getExpiryYear()) {
			return true;
		} else return false;
